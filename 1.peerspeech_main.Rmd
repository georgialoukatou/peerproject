...
write.table(wide_table1, file=paste0(path, "demuth_input_3.txt"), row.names = FALSE, col.names = TRUE)
...
write.table(table2, file=paste0(path, "demuth_input_4.txt"), row.names = FALSE, col.names = TRUE)
...
merged_speaker_info_demuth$role_adressee[merged_speaker_info_demuth$annotation1 %in% c("16901")] <- "Investigator" # these are probably typos
...
write.table(sum_demuth_register, file=paste0(path, "sum_demuth_register.txt"), row.names=F, col.names=T,  quote=F) 
...
write.table(wide_table2, file=paste0(path, "demuth_table2.txt"), row.names = FALSE, col.names = TRUE)
...
write.table(sum_demuth_register_speakers, file=paste0(path, "demuth_register.txt"), row.names = FALSE, col.names = TRUE)
...
names(largetotalregsp)<-c("keychild", "register", "speakers", "largetotalperchild")
...
sum_demuth_register_speakers$percentageperSpeakerRegisterChild<-sum_demuth_register_speakers$sumperSpeakerChild/sum_demuth_register_speakers$largetotalperchild*100
write.table(sum_demuth_register_speakers, file=paste0(path, "sum_demuth_register_speakers.txt"), row.names=F, col.names=T,  quote=F) 

table2.2<-unique(data.frame(sum_demuth_register_speakers$average, sum_demuth_register_speakers$keychild, sum_demuth_register_speakers$register, sum_demuth_register_speakers$speakers))
colnames(table2.2)<- c("mean", "sd", "keychild", "register", "speaker")
table2.2<-table2.2 %>% unite("mean_sd", mean:sd, )
write.table(table2.2, file=paste0(path, "demuth_table2.2.txt"), row.names = FALSE, col.names = TRUE)
```

################################# french corpus
```{r french preparation, eval=TRUE, include=TRUE, message=FALSE} 
#dataf<-read_sheet("https://docs.google.com/spreadsheets/d/1h8esdNOGHDxRr1aupgSKZh8tEmY66s4BJku2RpRTaVg/edit#gid=128166725")
dataf<-read.csv(file="/Users/admin/Downloads/French_corpus-Sheet1.csv", header=TRUE)
dataf$keychild <- NA
...
write.table(dataf, file=paste0(path,"dataf.txt"), row.names=F, col.names=T,  quote=F) 
...
#Sum the number of utterances per speaker category and keychild
aggregate(dataf$utterance_id,by=list( dataf$speaker, dataf$keychild, dataf$session),length )->sum_french
...
write.table(sum_french, file=paste0(path, "sum_french.txt"), row.names=F, col.names=T,  quote=F) 

table1<-unique(data.frame(sum_french$averagepersession, sum_french$keychild, sum_french$speaker))
colnames(table1)<- c("mean", "sd", "keychild", "speaker")
table1<-table1 %>% unite("mean_sd", mean:sd, )
wide_table1 <- table1 %>% spread(speaker, mean_sd)
#####TABLE1-FRENCH
write.table(wide_table1, file=paste0(path, "french_input_3.txt"), row.names = FALSE, col.names = TRUE)
...
total_french<-nrow(french) #35838 this is including keychild vocs
...
#export all ADS utterance into a text file
write.table(french_ad_directed_utts, file=paste0(path,"Frenchadultdirected.txt"), row.names=F, col.names=T,  quote=F) 
write.table(french_oc_directed_utts, file=paste0(path,"Frenchotherchilddirected.txt"), row.names=F, col.names=T,  quote=F) 
write.table(french_c_directed_utts, file=paste0(path,"Frenchchilddirected.txt"), row.names=F, col.names=T,  quote=F) 
...
write.table(sum_french_register, file=paste0(path, "sum_french_register.txt"), row.names=F, col.names=T,  quote=F) 


table2f<-unique(data.frame(sum_french_register$averagepersession, sum_french_register$keychild, sum_french_register$register))
...
write.table(wide_table2f, file=paste0(path, "french_table2.txt"), row.names = FALSE, col.names = TRUE)
...
write.table(sum_french_register_speakers, file=paste0(path, "french_register.txt"), row.names = FALSE, col.names = TRUE)
...
sum_french_register_speakers$percentageperSpeakerRegisterChild<-sum_french_register_speakers$sumperSpeakerChild/sum_french_register_speakers$largetotalperchild*100
write.table(sum_french_register_speakers, file=paste0(path, "sum_french_register_speakers.txt"), row.names=F, col.names=T,  quote=F) 




table2.2f<-unique(data.frame(sum_french_register_speakers$average, sum_french_register_speakers$keychild, sum_french_register_speakers$register, sum_french_register_speakers$speakers))
colnames(table2.2f)<- c("mean", "sd", "keychild", "register", "speaker")
table2.2f<-table2.2f %>% unite("mean_sd", mean:sd, )
write.table(table2.2f, file=paste0(path, "french_table2.2.txt"), row.names = FALSE, col.names = TRUE)

write.table(merged_speaker_info_demuth, file=paste0(path, "demuth.txt"), row.names = FALSE, col.names = TRUE, sep=";;")
write.table(merged_speaker_info_french, file=paste0(path, "french.txt"), row.names = FALSE, col.names = TRUE)
...
for (sp in speakers){
  for (child in keychildren){
    for (sess in sessionsAnais){
      ...
      sess<-gsub("/", "_", sess)
      write.table(french_keychild_directed_sp_child_sess, file=paste0(path, "utterances/French/", child, sp, sess, "CDS.csv"), row.names=F, col.names=T, sep="\t",  quote=F) 
    }
  }
}
...
for (sp in speakers){
  for (child in keychildren){
    for (sess in sessionsAnais){
      ...
      sess<-gsub("/", "_", sess)
      write.table(french_otherchild_directed_sp_child_sess, file=paste0(path, "utterances/French/", child, sp, sess, "OCDS.csv"), row.names=F, col.names=T, sep="\t",  quote=F) 
    }
  }
}
...
for (sp in speakers){
  for (child in keychildren){
    for (sess in sessionsAnais){
      ...
      sess<-gsub("/", "_", sess)
      write.table(french_adult_directed_sp_child_sess, file=paste0(path, "utterances/French/", child, sp, sess, "ADS.csv"), row.names=F, col.names=T, sep="\t",  quote=F) 
    }
  }
}
