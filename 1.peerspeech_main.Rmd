...
date              : 13/06/2020, 12/10/2020. 8/05/2021
...
First we are going to prepare the Demuth corpus for analysis. For this we will clean the data and clearly attribute to every utterance: a speaker, an addressee (in broad age/gender categories)
```{r demuth preparation, eval=TRUE, include=TRUE, message=FALSE}
#old googlesheet link 
#demuth <-read_sheet("https://docs.google.com/spreadsheets/d/1U-YheTr9fpi844p3pLSFujJOYAAG4BpJTv4Wn6Ki5JM/edit#gid=1937482996")
...
sum_demuth_register_speakers$percentageperSpeakerRegisterChild<-sum_demuth_register_speakers$largetotalperchildreg.x/sum_demuth_register_speakers$largetotalperchildkey*100
write.table(sum_demuth_register_speakers, file=paste0(path, "sum_demuth_register_speakers_noinv.txt"), row.names=F, col.names=T,  quote=F) 
...
dataf<-subset(dataf, !(dataf$speaker=="investigator")) #remove target child utterances
...
write.table(sum_french_register, file=paste0(path, "sum_french_register_noinv.txt"), row.names=F, col.names=T,  quote=F) 
...
write.table(sum_french_register_speakers, file=paste0(path, "french_register_noinv.txt"), row.names = FALSE, col.names = TRUE)
...
write.table(sum_french_register_speakers, file=paste0(path, "sum_french_register_speakers_noinv.txt"), row.names=F, col.names=T,  quote=F) 
...
write.table(merged_speaker_info_demuth, file=paste0(path, "demuth1.txt"), row.names = FALSE, col.names = TRUE, sep=";;")
write.table(merged_speaker_info_french, file=paste0(path, "french1.txt"), row.names = FALSE, col.names = TRUE, sep=";;")
