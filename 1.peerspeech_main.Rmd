...
author            : "Anon"
date              : 13/06/2020, 12/10/2020. 08/05/2021, 25/06/2021
...
derived="../for_reproduce/draft/"

#for text output derived from this script:
derivedtext="../for_reproduce/draft/"
...
french_adult_directed<-subset(merged_speaker_info_french, (merged_speaker_info_french$role_adressee=="Adult"|merged_speaker_info_french$role_adressee=="Mother"))
...
######SAVE ACTUAL UTTERANCES for text analysis - 2. conversational turns
write.table(merged_speaker_info_demuth, file=paste0(derivedtext, "demuth1_all.txt"), row.names = FALSE, col.names = TRUE, sep=";;")
write.table(merged_speaker_info_french, file=paste0(derivedtext, "french1_all.txt"), row.names = FALSE, col.names = TRUE, sep=";;")


######SAVE ACTUAL UTTERANCES for text analysis - 3. adjplusnoun
...
keychildren <- c("Litlhare")#"Hlobohang" "Neuoe" "Litlhare"
for (sp in speakers){
  for (child in keychildren){
    for (sess in sessionsLitlhare){   ####CHANGE SESSION NAME
     sess_<-gsub("/", "_", sess)
demuth_keychild_directed_ <- merged_speaker_info_demuth %>% filter(register=="CDS", 
                                                              speaker==sp, 
                                                              keychild==child,
                                                              session==sess)
if (nrow(demuth_keychild_directed_>0)){
      write.table(demuth_keychild_directed_, file=paste0(derivedtext, "Sesotho/", child, sp, sess_, "CDS.csv"), row.names=F, col.names=T, sep="\t",  quote=F)
 }
  demuth_otherchild_directed_ <- merged_speaker_info_demuth %>% filter(register=="OCDS", 
                                                              speaker==sp, 
                                                              keychild==child,
                                                              session==sess)
if (nrow(demuth_otherchild_directed_>0)){
   write.table(demuth_otherchild_directed_, file=paste0(derivedtext, "Sesotho/", child, sp, sess_, "OCDS.csv"), row.names=F, col.names=T, sep="\t",  quote=F)
      }
  
demuth_adult_directed_<- merged_speaker_info_demuth %>% filter(register=="ADS", 
                                                              speaker==sp, 
                                                              keychild==child,
                                                              session==sess)
if (nrow(demuth_adult_directed_>0)){
    write.table(demuth_adult_directed_, file=paste0(derivedtext, "Sesotho/", child, sp, sess_, "ADS.csv"), row.names=F, col.names=T, sep="\t",  quote=F) 
       }
     }
  }
}
...
for (sp in speakers){
  for (child in keychildren){
    for (sess in sessionsAnae){   ####CHANGE SESSION NAME
     sess_<-gsub("/", "_", sess)
french_keychild_directed_ <- merged_speaker_info_french %>% filter(register=="CDS", 
                                                              speaker==sp, 
                                                              keychild==child,
                                                              session_id==sess)
if (nrow(french_keychild_directed_>0)){
      write.table(french_keychild_directed_, file=paste0(derivedtext, "French/", child, sp, sess_, "CDS.csv"), row.names=F, col.names=T, sep="\t",  quote=F)
 }
  french_otherchild_directed_ <- merged_speaker_info_french %>% filter(register=="OCDS", 
                                                              speaker==sp, 
                                                              keychild==child,
                                                              session_id==sess)
if (nrow(french_otherchild_directed_>0)){
   write.table(french_otherchild_directed_, file=paste0(derivedtext, "French/", child, sp, sess_, "OCDS.csv"), row.names=F, col.names=T, sep="\t",  quote=F)
      }
  
french_adult_directed_<- merged_speaker_info_french %>% filter(register=="ADS", 
                                                              speaker==sp, 
                                                              keychild==child,
                                                              session_id==sess)
if (nrow(french_adult_directed_>0)){
    write.table(french_adult_directed_, file=paste0(derivedtext, "French/", child, sp, sess_, "ADS.csv"), row.names=F, col.names=T, sep="\t",  quote=F) 
     }
      }
  }
}
