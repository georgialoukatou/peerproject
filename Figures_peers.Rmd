...
h <- sum(tot_sum_demuth_register$nb_utt[tot_sum_demuth_register$keychild=="Hlobohang"]) #12634 
ts <- sum(tot_sum_demuth_register$nb_utt[tot_sum_demuth_register$keychild=="Tsebo"]) #9883
l <- sum(tot_sum_demuth_register$nb_utt[tot_sum_demuth_register$keychild=="Litlhare"])# 15326


ftot <-sum(tot_sum_french_register$nb_utt) #20917
anae <- sum(tot_sum_french_register$nb_utt[tot_sum_french_register$keychild=="Anae"]) #9677
anais <- sum(tot_sum_french_register$nb_utt[tot_sum_french_register$keychild=="Anais"])# 7212
th<- sum(tot_sum_french_register$nb_utt[tot_sum_french_register$keychild=="Theotime"]) 4028
...
````{r fig1, echo=F}
ggfillsesotho <-  ggplot(tot_sum_demuth_register, aes( y=nb_utt, x="", fill=speaker))+geom_bar(stat="identity", position="fill")+  labs(x="Sesotho", y="Number of utterances") + guides(fill=FALSE) + scale_fill_discrete(labels= c("Adults", "Investigator", "Mother", "Other children")) + blank_theme                                                                      
...
dsummaryspeaker_tot<- data_summary(simpled_tot, varname="nb_utt", 
                    groupnames=c("speaker", "keychild"))


ggrawsesotho <-  ggplot(dsummaryspeaker_tot, aes( y=as.numeric(nb_utt), x="", fill=speaker))+geom_bar(stat="identity", position="dodge")+  labs(x="Sesotho", y="Number of utterances") + guides(fill=FALSE) + scale_fill_discrete(labels= c("Adults", "Investigator", "Mother", "Other children")) + blank_theme 
...
```{r fig2}
gg2fillsesotho <-  ggplot(sum_demuth_register, aes( y=nb_utt, x="", fill=speaker))+geom_bar(stat="identity", position="fill")+  labs(x="Sesotho", y="Number of utterances") + guides(fill=FALSE) + scale_fill_discrete(labels= c("Adults", "Mother", "Other children")) + blank_theme

gg2fillfrench <- ggplot(sum_french_register, aes( y=nb_utt, x="", fill=speaker))+geom_bar(stat="identity", position="fill")+  labs(x="Sesotho", y="Number of utterances")   + scale_fill_discrete(labels= c("Adults", "Mother", "Other children")) + blank_theme

ggarrange(gg2fillsesotho, gg2fillfrench, ncol=2, nrow=1, common.legend = TRUE, legend="right",  labels = c("Sesotho", "French"))

gg2rawsesotho <-  ggplot(sum_demuth_register, aes( y=nb_utt, x="", fill=speaker))+geom_bar(stat="identity", position="dodge")+  labs(x="Sesotho", y="Number of utterances") + guides(fill=FALSE) + scale_fill_discrete(labels= c("Adults","Mother", "Other children")) + blank_theme

gg2rawfrench <- ggplot(sum_french_register, aes( y=nb_utt, x="", fill=speaker))+geom_bar(stat="identity", position="dodge")+  labs(x="Sesotho", y="Number of utterances")   + scale_fill_discrete(labels= c("Adults", "Mother", "Other children")) + blank_theme

ggarrange(gg2rawsesotho, gg2rawfrench, ncol=2, nrow=1, common.legend = TRUE, legend="right",  labels = c("Sesotho", "French"))

kch_french <- ggplot(sum_french_register, aes( y=nb_utt, x=keychild, fill=speaker))+geom_bar(stat="identity", position="dodge")+  labs(x="Keychild", y="Number of utterances")  + scale_fill_discrete(labels= c("Adults", "Mother", "Other children")) + blank_theme
kch_demuth <- ggplot(sum_demuth_register, aes( y=nb_utt, x=keychild, fill=speaker))+geom_bar(stat="identity", position="dodge")+  labs(x="Keychild", y="Number of utterances")+ guides(fill=FALSE)  + scale_fill_discrete(labels= c("Adults", "Mother", "Other children")) + blank_theme
