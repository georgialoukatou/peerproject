...
````{r fig1, echo=F}
ggfillsesotho <-  ggplot(tot_sum_demuth_register, aes( y=nb_utt, x="", fill=speaker))+geom_bar(stat="identity", position="fill")+  labs(x="Sesotho", y="Number of utterances") + guides(fill=FALSE) + scale_fill_discrete(labels= c("Adults", "Investigator", "Mother", "Other children")) + blank_theme + geom_text(aes(label = paste0(nb_utt,'%')),colour = 'white', position=position_stack(vjust=0.5)) 
