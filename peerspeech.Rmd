...
wells_input$speaker[wells_input$ROLE == "Mother"] <- "mother"
wells_input$speaker[grep("Sister|Brother|Sibling", wells_input$ROLE)]  <- "sibling"
wells_input$speaker[grep("Child|Playmate|child", wells_input$ROLE)]  <- "peer"
wells_input$speaker[grep("Adult|Uncle|Grandmother|Family_Friend|Visitor|Aunt|Father", wells_input$ROLE)]  <- "adult"


write.table(wells_input, file=paste0("wells_input.txt"), row.names=F, col.names=T,  quote=F) 
...
read.table("wells_input.txt",header=T)->wells_input
