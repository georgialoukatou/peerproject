...
data<-read.csv(file="sesotho_CDS_complete.csv", header=TRUE,  na.strings=c("","NA"," "))
...
data$speaker[grep("Sister|Brother|Sibling", data$speaker_role_raw)]  <- "sibling"
data$speaker[grep("Child|Playmate|child|Teenager|Cousin", data$speaker_role_raw)]  <- "peer"
...
#save changes
write.table(data, file=paste0("demuth_input.txt"), row.names=F, col.names=T,  quote=F) 
...
write.table(sum_demuth, file=paste0("demuth_speakers.txt"), row.names=F, col.names=T,  quote=F) 
...
nrow(demuth) #48403 this is including keychild vocs this is including keychild vocs
demuth<-subset(demuth, !(demuth$speaker_role_raw=="Target_Child")) #remove target child utterances
nrow(demuth) #37843
...
merged_speaker_info_demuth$role_adressee[merged_speaker_info_demuth$annotation1 %in% c("C","N","O")] <- "unknown" # these letters idk what they mean
...
demuth_adult_directed<-subset(merged_speaker_info_demuth, (merged_speaker_info_demuth$role_adressee=="Adult"| merged_speaker_info_demuth$role_adressee=="Father"|merged_speaker_info_demuth$annotation1=="Mother"| merged_speaker_info_demuth$annotation1=="Uncle"| merged_speaker_info_demuth$role_adressee=="Grandmother"|merged_speaker_info_demuth$role_adressee=="Investigator"))

# Utterances addressed to others - children
demuth_otherchild_directed<-subset(merged_speaker_info_demuth, (merged_speaker_info_demuth$role_adressee=="Brother"| merged_speaker_info_demuth$role_adressee=="Cousin"|merged_speaker_info_demuth$annotation1=="Playmate"| merged_speaker_info_demuth$annotation1=="Sister"| merged_speaker_info_demuth$role_adressee=="Teenager"))

# Utterances addressed to keychild 
demuth_keychild_directed<-subset(merged_speaker_info_demuth, (merged_speaker_info_demuth$role_adressee=="keychild"| merged_speaker_info_demuth$role_adressee=="self"))
...
merged_speaker_info_demuth$register[merged_speaker_info_demuth$role_adressee == "Investigator"] <- "ADS"
merged_speaker_info_demuth$register[grep("Sister|Brother|Sibling", merged_speaker_info_demuth$role_adressee)]  <- "OCDS"
merged_speaker_info_demuth$register[grep("Playmate|Teenager|Cousin", merged_speaker_info_demuth$role_adressee)]  <- "OCDS"
...
#   geom_bar(stat="identity", position="dodge")
 ...
 ggplot(sum_demuth, aes( y=nb_utt, x=keychild, fill=speaker))+
  geom_bar(stat="identity", position="dodge")+  labs(x="Keychild", y="Number of utterances")


# wells_addresse<- ggplot(sum_wells_addressee, aes( y=nb_utt, x=keychild, fill=addresse))+
#   geom_bar(stat="identity", position="dodge")
 ggplot(sum_demuth_register, aes( y=nb_utt, x=keychild, fill=input))+
  geom_bar(stat="identity", position="dodge")
...
#   geom_bar(stat="identity", position="dodge") +  labs(x="Wells Corpus", y="Number of utterances") + guides(fill=guide_legend(title=NULL))
 Hlobohang <- ggplot(subset(sum_demuth_register, keychild %in% c("Hlobohang") ), aes( y=nb_utt, x=input, fill=speaker))+
  geom_bar(stat="identity", position="dodge") +  labs(x="Hlobohang", y="")  + guides(fill=FALSE)
 
  Litlhare <- ggplot(subset(sum_demuth_register, keychild %in% c("Litlhare") ), aes( y=nb_utt, x=input, fill=speaker))+
  geom_bar(stat="identity", position="dodge") +  labs(x="Litlhare", y="Number of utterances")  + guides(fill=guide_legend(title=NULL))
  
   Tsebo <- ggplot(subset(sum_demuth_register, keychild %in% c("Tsebo") ), aes( y=nb_utt, x=input, fill=speaker))+
  ...
  geom_bar(stat="identity", position="fill") +  labs(x="Hlobohang", y="")  + guides(fill=FALSE)
...
ggplot(sum_demuth_register, aes( y=nb_utt, x=input, fill=speaker))+
  geom_bar(stat="identity", position="dodge") +  labs(x="", y="")  + guides(fill=guide_legend(title=NULL))
