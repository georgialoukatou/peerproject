...
lang<-"English"
wells<-read.csv(file="wells.csv", header=T) #11,866
wells[6]<-NULL
...
well_adult_directed<-subset(well_adult_directed, !(well_adult_directed$speaker=="Target_Child"))
#subset all utterances marked as ADS
...
wells_CDS<- ggplot(wells_fig, aes( y=as.numeric(nb_utt), x=input, fill=speaker))+
  geom_bar(stat="identity", position="fill")
demuth_CDS<- ggplot(demuth_fig, aes( y=as.numeric(nb_utt), x=input, fill=speaker))+
  geom_bar(stat="identity", position="fill")
...
#panel
 ...
 dodge <- multiplot (wells_CDS, demuth_CDS, cols=1) 
