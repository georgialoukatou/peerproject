...
write.table(sum_demuth, file=paste0(path, "demuth_speakers.txt"), sep="") 
...
dataf<-read.csv(file="French_corpus.csv", header=TRUE)
...
write.table(sum_french, file=paste0(path, "french_speakers.txt"), sep = "") 
french<-dataf
...
```{r demuth cleaning_1, eval=TRUE, include=TRUE, message=FALSE}
...
#total utterances in demuth corpus
total_demuth<-length(demuth$utterance_id) # 37843
```


```{r demuth compute_addressee}
...
write.table(sum_demuth_register, file=paste0(path, "demuth_register.txt"), sep="") 
```
...
# Utterances addressed to keychild 
# this shoud be redundant to demuth_CD
...
##Others directed utterances ####

merged_speaker_info_demuth<-subset(merged_speaker_info_demuth, !(merged_speaker_info_demuth$speaker=="investigator"))


# Utterances addressed to adults
...
# Utterances addressed to other children
...
### THIS ONE DOESNT RUN 
#Need to define demuth_keychild_directed_fig
...
#     write.table(demuth_keychild_directed_fig, file=paste0(child, sp,"CDS.csv"), row.names=F,  sep="\t", col.names=T,  quote=F) 
#   }
# }    
...
```{r french cleaning, eval=TRUE, include=TRUE, message=FALSE}
...
write.table(sum_french_register, file=paste0(path, "french_register.txt"), sep="") 
```
...
colnames(sum_french_OCDS) <- c( "speaker", "keychild", "nb_utt")
...
##subset all utterances marked as ADS & OCDS = OVERHEARD SPEECH####
...
q1<-rbind(q1f_, q1_)
model<-lmer(formula= ratio ~ speaker * language + (1 |session) + (1 + speaker|targetchild), data=q1) ##########first model
...
q2<-rbind(q2f_, q2_)
model2<-lmer(formula= ratio ~ speaker * language *register+  (1 |session)+ (1 + speaker*language*register|session) + (1 + speaker*language*register|targetchild), data=q2) ##########first model
